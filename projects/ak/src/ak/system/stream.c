...
{
  uint8_t buf[2];
  ak_stmerr err = ak_stm_read(stm, buf, 2);
  if (err)
    return err;
...
{
  uint8_t buf[4];
  ak_stmerr err = ak_stm_read(stm, buf, 4);
  if (err)
    return err;
...
{
  uint8_t buf[8];
  ak_stmerr err = ak_stm_read(stm, buf, 8);
  if (err)
    return err;
