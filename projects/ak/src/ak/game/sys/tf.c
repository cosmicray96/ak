...
ak_sys_tf_make(ak_alct alct)
{
  ak_sys_tf stf = { 0 };
  stf.alct = alct;
  stf.it = ak_wv_itdfspre_make_empty(alct);
  stf.gmat3s =
                 ...
                 ak_wcb* wcb)
{
  ...
  {
    ak_assert(e);
    ak_assert(!ak_wv_ett_parent(wv, e));

    ak_tf2d_t tf = ak_wv_comp_tf2d(wv, e);
    ak_mat3 mat = ak_tf2d_to_mat3x3(&tf);
    ak_spa_insert(&stf->gmat3s, e, &mat);
    ak_wcb_comp_gmat3_add(wcb, e, mat);
  }
  ...
  while (1) {
    ...
    ak_assert(ak_wv_comp_tf2d_exist(wv, e));
    ak_assert(ak_wv_comp_gmat3_exist(wv, e));
